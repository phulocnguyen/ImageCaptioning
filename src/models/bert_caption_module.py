...
from src.models.components import BertCaptioningNet
from src.utils.decode import greedy_search, batch_greedy_search, beam_search_decoding


class BertCaptionModule(LightningModule):
    ...
    def model_step(
        ...
        scores, encoded_captions, decode_lengths, alphas, sort_ind = self.forward(images, sequences)

        loss = self.criterion(
            pack_padded_sequence(scores, decode_lengths, batch_first=True)[0],
            pack_padded_sequence(targets[:, 1:], decode_lengths, batch_first=True)[0]
        )
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        module: BertCaptionModule = hydra.utils.instantiate(cfg)
